...
async function loadTopZones(borough = '') {
  const url = borough
    ? `${API_BASE_URL}/trips/top-zones?borough=${borough}`
    : `${API_BASE_URL}/trips/top-zones`;
...
async function initCharts() {
  try {
    ...
    console.error(err);
    setStatus("Could not load chart data. Is the Flask backend running on port 5000?", true);
  }
}
